...
if __name__ == "__main__":
  ...
  ax.set_xlabel('Pulse + 100ms low power compute',fontsize=10,fontweight=300)
...
arrowprops=dict(arrowstyle="-", color='k',lw=1))
  ...
  ax.legend(ncol=3,fontsize=12,loc=(0.06, 0.15))
  # Plot vsafe
  #ax2 = ax.twinx()
  #conservative_vsafes = np.divide(np.multiply(conservative_vsafes_used,VRANGE),4096)
  #culpeo_vsafes = np.divide(np.multiply(culpeo_vsafes_used,VRANGE),4096)
  ...
  plt.show()
  fig.savefig('synthetic_loads_plot_abstract.pdf',format='pdf',bbox_inches='tight')
