...
names = ['Energy-V','Catnap', '$\it{Culpeo-PG}$', '$\it{Culpeo-R}$']
dict_keys = [ 'energy', 'catnap', 'culpeo', 'dynamic' ]
...
colors = ["#f7f7f7","#92BA92", "#92c5de",'#355764',"#0571b0"]
...
if __name__ == "__main__":
  ...
  for expt_count,expt_id in enumerate(expts_to_use):
    ...
    if culpeo_expts[expt_id]['avg_min'] > VMIN:
      ...
      if (catnap_expts[expt_id]['avg_min'] < V_HARD_FAIL):
        ...
        expt_results['diffs'][expt_count]['catnap'] = catnap_expts[expt_id]['avg_min']
      ...
      if (expt_id in dynamic_expts.keys()) == False:
        ...
        if (dynamic_expts[expt_id]['avg_min'] < V_HARD_FAIL):
          ...
          if (dynamic_expts[expt_id]['avg_start'] > VHIGH):
            ...
            expt_results['diffs'][expt_count]['dynamic'] = dynamic_vsafes[expt_id]
          else:
            dynamic_diffs.append(dynamic_expts[expt_id]['avg_min'])
            expt_results['diffs'][expt_count]['dynamic'] = dynamic_expts[expt_id]['avg_min']
      # Handle culpeo
      ...
      expt_results['diffs'][expt_count]['culpeo'] = culpeo_expts[expt_id]['avg_min']
      print(culpeo_diffs)
      if (culpeo_expts[expt_id]['avg_min'] < VMIN):
        print("Fail: ",expt_id)
      ...
      expt_results['vsafes'][expt_count] = {
         'energy': energy_expts[expt_id]['avg_start'],
         'catnap': catnap_expts[expt_id]['avg_start'],
         'dynamic': dynamic_expts[expt_id]['avg_start'],
         'culpeo': culpeo_expts[expt_id]['avg_start']}

  Xs = np.arange(len(names))
  fig, ax = plt.subplots()
  
  gesture_xs = np.arange(len(names)) - bar_width
  gesture_vsafes = []
  gesture_diffs = []
  for key in dict_keys:
    gesture_vsafes.append( expt_results['vsafes'][0][key] ) 
    gesture_diffs.append( expt_results['diffs'][0][key] ) 
  gesture_delta = np.subtract(gesture_vsafes,gesture_diffs) - CS_diff
  plt.errorbar(gesture_xs,gesture_vsafes,gesture_delta,elinewidth=LW,fmt='.',
  ecolor=colors[1],color=colors[1],\
  marker='_',mfc=colors[1],uplims=True,capsize=CS,label='Gesture',alpha=1)
  
  ble_xs = np.arange(len(names))
  ble_vsafes = []
  ble_diffs = []
  for key in dict_keys:
    ble_vsafes.append( expt_results['vsafes'][1][key] ) 
    ble_diffs.append( expt_results['diffs'][1][key] ) 
  ble_delta = np.subtract(ble_vsafes,ble_diffs) - CS_diff
  plt.errorbar(ble_xs,ble_vsafes,ble_delta,elinewidth=LW,fmt='.',
  ecolor=colors[2],color=colors[2],\
  marker='_',mfc=colors[2],uplims=True,capsize=CS,label='BLE',alpha=1)
  
  mnist_xs = np.arange(len(names)) + bar_width
  mnist_vsafes = []
  mnist_diffs = []
  for key in dict_keys:
    mnist_vsafes.append( expt_results['vsafes'][2][key] ) 
    mnist_diffs.append( expt_results['diffs'][2][key] ) 
  mnist_delta = np.subtract(mnist_vsafes,mnist_diffs) - CS_diff
  plt.errorbar(mnist_xs,mnist_vsafes,mnist_delta,elinewidth=LW,fmt='.',
  ecolor=colors[3],color=colors[3],\
  marker='_',mfc=colors[3],uplims=True,capsize=CS,label='MNIST',alpha=1)
  ''' 
  ...
  plt.yticks(np.arange(0,3,0.5),fontsize=FS,fontweight=300)
  #ax.annotate('', xy=(.5,-.11), xycoords='axes fraction', xytext=(1, -0.11),
#arrowprops=dict(arrowstyle="-", color='k',lw=1))

  plt.axhline(VMIN,color="#B20600",linestyle='--',lw=3.5)
  plt.axhline(VMAX,color="#91cf60",linestyle='--',lw=3.5)
  for x in Xs:
    plt.axvline(x+2.5*bar_width,color='black',alpha=.8,lw=1, linestyle=':')
  boldness = 300
  
  vsafe_pos = (1.5+0.3*bar_width,VMAX-0.2)
  ax.annotate("$V_{safe}$", vsafe_pos, \
  fontsize=FS,fontweight=boldness,ha='center', va='bottom', color='k')
  ax.arrow(vsafe_pos[0],vsafe_pos[1],gesture_xs[2]-vsafe_pos[0],gesture_vsafes[2]-vsafe_pos[1], color='k')

  vmin_pos = (1.5+0.3*bar_width,VMIN+0.2)
  ax.annotate("$V_{min}$", vmin_pos, \
  fontsize=FS,fontweight=boldness,ha='center', va='top', color='k')
  ax.arrow(vmin_pos[0],vmin_pos[1],gesture_xs[2]-vmin_pos[0],gesture_diffs[2]-vmin_pos[1]-0.035, color='k')

  ax.annotate("$V_{high}$",(3+0.9*bar_width,VMAX ),va='bottom',fontsize=FS,fontweight=boldness, color='#224B0C')
  ax.annotate("$V_{off}$",(3+1.25*bar_width,VMIN - .05),va='top',fontsize=FS,fontweight=boldness, color='#B20600')
  ax.set_xticklabels(names,fontsize=FS)
  ax.legend(ncol=1,fontsize=FS-4,loc='lower right',\
  bbox_to_anchor=[1.0,0.1])#,bbox_to_anchor=[0.4, 0])
  ax.tick_params(labelsize=FS)
  plt.fill_between([-0.45,0.45],VMAX+0.3,color='#EE5007', alpha=0.1)
  plt.fill_between([0.55,1.45],VMAX+0.3,color='#EE5007', alpha=0.1)
  plt.fill_between([1.55,2.45],VMAX+0.3,color='#224B0C', alpha=0.1)
  plt.fill_between([2.55,3.45],VMAX+0.3,color='#224B0C', alpha=0.1)
  ax.xaxis.tick_top()
  # Plot vsafe
  #ax2 = ax.twinx()
  ...
  #plt.minorticks_on()
  ax.grid(which="both",axis="y", linewidth=2, linestyle='--', alpha=0.5)
  #plt.ylim(-.1,. 4)
  ratio = 1/2
  xleft, xright = ax.get_xlim()
  ybottom, ytop = ax.get_ylim()
  #ax.set_aspect(abs((xright-xleft)/(ybottom-ytop))*ratio)
  ax.set_ylim(0,VMAX+0.3)
  ax.set_xlim(-0.5,3.5)
  plt.show()
